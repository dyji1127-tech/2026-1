## Homework #4: Recursions and Stacks

**Due:  May 14 (Thursday), 2026**
...
For problems (1) and (2), you can make use of `Integer.parseInt("7")` static method to convert string to integer, and String's `substring()` instance method to select substrings. Make sure to consult the corresponding Java docs for each method to understand their usages.
...
The entry point and the exit point will be chosen randomly as long as the
respective entries is 0’s. One possible choices are index positions `[0,0]` and `[M-1,N-1]`, respectively. That is, the entry point is `[0,0]` and the exit point is `[3,4]`.

For the sake of simplicity, all the sample mazes are assumed to have 0's in
the entry and exit positions. The rest of the entries in the matrix will be given at random.
...
1. Your program **must** search the path *in the fixed order*: **`east`, `west`, `south`, `north`**.
